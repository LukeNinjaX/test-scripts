import { ContextKey, ConvertUtil, ErrLoadRuntimeCtxValue } from '@artela/aspect-libs/common';
import { MutableAspectValue } from '@artela/aspect-libs/components/aspect/aspect-state-interface';
import { SetNameSpace, StringData } from '@artela/aspect-libs/proto';
import { Protobuf } from 'as-proto';
import { RuntimeContextApi } from '@artela/aspect-libs/hostapi';
...
export class TransientStorageValue<T> implements MutableAspectValue<T> {
  ...
  set<T>(value: T): bool {
    ...
    let success = runtimeContext.set(SetNameSpace.SetAspectContext, this.key, dataStr);
    if (success) {
      this.loaded = false;
    }
    return success;
